...
<script type="text/javascript">
        ...
        $("#addActivityBtn").click(function () {
            if($("input[name='checks']:checked").length === 0){
                ...
                for (var i = 0; i < $("input[name='checks']:checked").length; i++) {
                    actIds[i] = $("input[name='checks']:checked")[i].value;
                }
                $.ajax({
					url:"clue/bundActivity",
					data:{
						clueId:"${clue.id}",
						actIds:actIds
					},
    ...
    //按名称查询所有市场活动
    function bundActivity(actName) {
        $.ajax({
            url:"clue/addActivityList",
            data:{name:actName,id:"${clue.id}"},
            type:"get",
            dataType:"json",
            success:function(result){
                var htmlText = "";
                ...
                $.each(result,function (i,n) {
                        htmlText += '<tr>';
                        htmlText +=     '<td><input name="checks" type="checkbox" value="'+n.id+'"/></td>';
    ...
    //刷新市场活动列表
    function ajaxUpdateActivityList(clueId) {
        $.ajax({
            url:"clue/updateActivity",
			data:{clueId:clueId},
			type:"get",
			dataType:"json",
			success:function (result) {
				var htmlText = "";
				$("#clue_activityList").html(htmlText);
				$.each(result,function (i,n) {
					htmlText += '<tr>';
					htmlText += '	<td>'+n.name+'</td>';
					htmlText += '	<td>'+n.startDate+'</td>';
					htmlText += '	<td>'+n.endDate+'</td>';
					htmlText += '	<td>'+n.owner+'</td>';
					htmlText += '	<td><a href="javascript:void(0);" onclick="unbundActivity(\''+n.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					htmlText += '</tr>';
				});
				$("#clue_activityList").html(htmlText);
			}
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="addActivityBtn">关联</button>
				</div>
			</div>
		</div>
	</div>
